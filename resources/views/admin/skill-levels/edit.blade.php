...
<div class="px-4 sm:px-0">
    ...
    <nav class="mb-4 text-sm">
        <ol class="flex items-center space-x-2 text-gray-500 dark:text-gray-400">
            ...
            <li><span class="mx-2">/</span></li>
            <li class="text-gray-900 dark:text-white font-medium">Edit Level {{ $skillLevel->level_number }}</li>
        </ol>
    </nav>

    <div class="sm:flex sm:items-center sm:justify-between">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Skill Level: {{ $skillLevel->name }}</h1>
    ...
    <div class="mt-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
        <form action="{{ route('admin.skill-levels.update', $skillLevel) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')

            @include('admin.skill-levels._form', ['skillLevel' => $skillLevel])
